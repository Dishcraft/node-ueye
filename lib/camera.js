...
const WSIZE = (process.platform === 'win32' ? 2 : 4);

function makeWideString(str)
{
    const size = (str.length + 1) * WSIZE;
    let buffer = Buffer.alloc(size);
    const pbuffer = buffer.ref();

    ref.set(pbuffer, 0, str, wchart.string);
...
Camera.prototype.init = function (id) {
    const self = this;

    return new Promise((resolve, reject) => {
        const hCam = ref.alloc('uint', Number(id) || 0);
...
Camera.prototype.loadParameterFile = function (paramFile) {
    const self = this;

    return new Promise((resolve, reject) => {
        const fileBuf = makeWideString(paramFile);
        return binding.is_ParameterSet.async(self.hCam, defines.IS_PARAMETERSET_CMD_LOAD_FILE, fileBuf, fileBuf.length, (err, nRet) => {
            if (err || nRet !== defines.IS_SUCCESS) {
                return reject(new Error(`is_ParameterSet: ${err || nRet}`));
            }
...
Camera.prototype.imageFile = function (options) {
    ...
    return new Promise((resolve, reject) => {
        options.pwchFileName = makeWideString(options.pwchFileName);
